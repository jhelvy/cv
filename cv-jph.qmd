...
- **Clean Technology Development & Adoption**: Study how consumers, firms, markets, and policy affect the nature & pace of transitioning to sustainable energy and transportation technologies.
...
grants_ex_pi <- grants %>% 
      filter_grants(external = 1, pi = 1, affiliated = 0)

total_ex_pi <- scales::dollar(sum(grants_ex_pi$budget_total))
```

**Total under Helveston management**: `r total_ex_pi`
...
```{r}
grants_ex_copi <- grants %>% 
      filter_grants(external = 1, pi = 0, affiliated = 0)

total_ex_copi <- scales::dollar(sum(grants_ex_copi$budget_jph))
```

**Total under Helveston management**: `r total_ex_copi`
...
grants_affiliated <- grants %>% 
    filter_grants(external = 1, pi = 0, affiliated = 1) 

total_affiliated <- scales::dollar(sum(grants_affiliated$budget_total)) 
```

**Total affiliated**: `r total_affiliated`
...
grants_affiliated %>% 
    mutate(
      citation = paste(
        sponsor, title, colabs, period, sep = '. '
      )
    ) %>% 
    pull(citation) %>%
    make_ordered_list()
...
grants_internal <- grants %>% 
    filter_grants(external = 0, pi = 1, affiliated = 0)

total_internal <- scales::dollar(sum(grants_internal$budget_total))
```

**Total under Helveston management**: `r total_internal`
...
*Graduate student advised by Helveston

**Undergraduate student advised by Helveston

_Note on author order: In my field, it is conventional that the last author denotes intellectual direction by the senior author when advised student(s) are engaged on the study. On non–student papers, authorship effort is denoted in standard order_.
...
## E. Conference Panel Organizer / Chair
...
**Total Media Appearances**: `r media_counts`
...
teaching <- get_cv_sheet('teaching') %>%
  ...
  mutate(
    with = ifelse(is.na(with), '', paste0(', with ', with)),
    title = paste0(enquote(title), with),
    title = ifelse(is.na(host), title, paste(title, host, sep = '. ')),
    date = paste0(format(as.Date(date), format = "%b %d, %Y"), '.'),
...
Completed 71 journal reviews and 13 conference paper reviews (2013 - present)
...
df %>%
  kbl(col.names = NULL, format = "latex", booktabs = TRUE) %>%
